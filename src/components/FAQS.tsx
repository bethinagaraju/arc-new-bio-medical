...
const FAQS = () => {
  ...
  // Find conference dates from importantDates
  const conferenceDates = data?.importantDates?.find(date => date.dateType === "Conference Dates")?.date || "July 28-30, 2026";

  // Find other important dates
  const earlyBirdRegistrationCloses = data?.importantDates?.find(date => date.dateType === "Early Bird Registration Closes")?.date || "February 25, 2026";
  const abstractSubmissionCloses = data?.importantDates?.find(date => date.dateType === "First Round of Abstract Submission Closes")?.date || "February 26, 2026";
  const notificationOfAcceptance = data?.importantDates?.find(date => date.dateType === "Notification of Acceptance")?.date || "February 2, 2026";
  const finalPaperSubmission = data?.importantDates?.find(date => date.dateType === "Final Paper Submission")?.date || "March 16, 2026";
  const submissionDeadline = data?.importantDates?.find(date => date.dateType === "Submission Deadline")?.date || "January 26, 2026";
  const faqItems = [
    {
      title: "WHEN & WHERE",
      content: (
        <div className="space-y-4">
          <div>
            ...
            <p>ICHIS 2026 will be held during <strong>{conferenceDates}</strong>.</p>
          </div>
          <div>
            <h4 className="font-bold text-gray-900">Where will the conference be held?</h4>
            <p>
              ICHIS 2026 will be held in Hybrid mode, participants can join <strong>In Person</strong> at <strong>Seoul, South Korea</strong> or <strong>Virtually</strong> from home or work.
            </p>
          </div>
        </div>
      ),
      ...
      content: (
        <div className="space-y-4">
          <div>
            ...
            <p>Registration for the ICHIS 2026 Conference is <strong>now open</strong>.</p>
          </div>
          <div>
            <h4 className="font-bold text-gray-900">Where do I go to register for the ICHIS 2026 conference?</h4>
